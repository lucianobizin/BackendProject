...
export default class CartsDao {
    ...
    #path = "./src/dao/filesyste/Carts.json";
    ...
    async create(cart) {
        ...
        for (let c of propertyList) {

            if (!Object.keys(cart).includes(c)) {

                return `The cart you are entering cannot be added due to the fact that property ${c} is not filled in`;
        ...
        try {
            ...
            if (fs.existsSync(this.#path)) {
                ...
                const checkingCode = products.find(prod => prod.code === cart.code);

                if (!checkingCode) {

                    const lastProduct = products[products.length - 1];

                    cart.id = lastProduct ? lastProduct.id + 1 : 1;
